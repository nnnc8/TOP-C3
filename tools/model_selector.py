...
def copy_staged_onnx(staging_dir: Path, models_dir: Path) -> None:
...
def install_model(
  ...
  with tempfile.TemporaryDirectory(prefix="openpilot-model-") as tmp:
    ...
    stage_model_files(model, staging_dir)
    layout = inspect_staged_bundle(repo_root, staging_dir, python_bin)
    backup_dir = backup_current_bundle(models_dir, model_id)

    try:
      copy_staged_onnx(staging_dir, models_dir)
      if not skip_rebuild:
        rebuild_artifacts(repo_root, models_dir, python_bin)
    except Exception:
